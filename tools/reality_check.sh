#!/usr/bin/env bash
# Usage: ./reality_check.sh [--no-install] [--no-external] <domain[:port]>
set -euo pipefail
...
while [ $# -gt 0 ]; do
  case "$1" in
    ...
    --no-external) NO_EXTERNAL=1; shift ;;
    -h|--help) echo "Usage: $0 [--no-install] [--no-external] <domain[:port]>"; exit 0 ;;
    --) shift; break ;;
...
if ! [[ "$DOMAIN" =~ ^[A-Za-z0-9._-]+$ || "$DOMAIN" =~ ^[0-9A-Fa-f:.]+$ ]]; then
  echo -e "${RED}Error: invalid domain/IP input.${RESET}"
  exit 1
fi

if [ "$PORT" -lt 1 ] || [ "$PORT" -gt 65535 ]; then
  echo -e "${RED}Error: port must be in range 1..65535.${RESET}"
  exit 1
fi
